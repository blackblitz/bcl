...
for dataset_name, Dataset in zip(['growingiris2', 'splitiris2'], [GrowingIris, SplitIris]):
    dataset = Dataset(nx=2)
    for model_name in ['sr', 'nn']:
        ...
        for i, (xlabel, algo, kwargs) in enumerate(zip(
            ...
            for j, (loss, state, x, y) in enumerate(
                algo(1000, state_init, loss_sce, dataset, **kwargs)
            ):
                plot_pred_softmax(axes[j, i], state)
                scatter = plot_xy(axes[j, i], x, y)
                axes[j, i].set_title(f'AA: {aa_softmax(j, state, dataset):.4f}')
                if i == 0:
                    axes[j, i].set_ylabel(f'Time {j + 1}')
            axes[-1, i].set_xlabel(xlabel)
        fig.savefig(f'plots/{dataset_name}_{model_name}_pred.png')
        ...
        for i, (xlabel, algo, kwargs) in enumerate(zip(
            ...
            for j, (loss, state, x, y) in enumerate(
                ...
                if i == 0:
                    axes[j, i].set_ylabel(f'Time {j + 1}')
            axes[-1, i].set_xlabel(xlabel)
        fig.savefig(f'plots/{dataset_name}_{model_name}_pred_ewc.png')
        ...
        for i, (xlabel, algo, kwargs) in enumerate(zip(
            ...
            for j, (loss, state, x, y) in enumerate(
                ...
                if i == 0:
                    axes[j, i].set_ylabel(f'Time {j + 1}')
            axes[-1, i].set_xlabel(xlabel)
        fig.savefig(f'plots/{dataset_name}_{model_name}_pred_nc.png')
