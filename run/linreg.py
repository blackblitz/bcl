...
from dataseqs.linreg import DupLinReg
...
def plot_loss(ax, state, loss, x, y, lim=20.0, vmax=20000.0):
...
for i, (xlabel, algo, kwargs) in enumerate(zip(
    ...
    for j, (state, loss, dataset) in enumerate(
        algo(1000, state_init, loss_sse, dataseq, **kwargs)
    ):
        x, y = next(iter(
            DataLoader(
                dataset,
