...
def bernne(p):
    """Calculate the normalized entropy of Bernoulli random variables."""
    return (entr(p) + entr(1 - p)) / jnp.log(2)


def catne(p):
    """Calculate the normalized entropy of categorical random variables."""
...
class MAPPredictor:
    ...
    def ne(self, xs):
        """Calculate the normalized entropy of predictions."""
        match self.mspec.nll:
            case NLL.SIGMOID_CROSS_ENTROPY:
                return bernne(self(xs, decide=False))
            case NLL.SOFTMAX_CROSS_ENTROPY:
                return catne(self(xs, decide=False))
...
class BMAPredictor:
    ...
    def ne(self, xs):
        ...
        match self.mspec.nll:
            case NLL.SIGMOID_CROSS_ENTROPY:
                return bernne(self.sample(xs).mean(axis=0))
            case NLL.SOFTMAX_CROSS_ENTROPY:
                return catne(self.sample(xs).mean(axis=0))
