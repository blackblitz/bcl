"""Script for Split Iris 1 - radius."""
...
for i, model in enumerate(tqdm([lreg, nnet], unit='model')):
    ...
    trainers = [
        ...
        *(NeuralConsolidation(
            state_main,
            {
                'precision': 0.1, 'radius': radius, 'size': 1024,
                'nsteps': 1000, 'state': state_consolidator, 'reg': 0.1
            },
            **trainer_kwargs
    ...
    for j, trainer in enumerate(
        ...
        if j > 0:
            axes[j * 2 + 1].set_xlabel(
                '$r={}$'.format(trainer.hyperparams.get('radius'))
            )
axes[1].set_xlabel('Joint training')
axes[0].cax.colorbar(mesh)
fig.savefig('plots/splitiris1_loss_radius.png')
